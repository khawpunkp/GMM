...
watch(sortOption, (value) => {
  if (category.value)
    localStorage.setItem(SORT_KEY_PREFIX + category.value.slug, value);
});
...
async function loadForSlug(slug: string) {
  ...
  try {
    ...
    loadSortOption(slug);
    await Promise.all([
      modsStore.fetchByCategory(found.id),
...
async function handleQuickImport() {
  ...
  const imported = await quickImportMod({ categoryId: category.value.id });
  if (imported) await modsStore.fetchByCategory(category.value.id);
}
