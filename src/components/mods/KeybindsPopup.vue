...
<template>
  ...
  >
    <div
      class="w-11/12 max-w-120 max-h-[85vh] overflow-y-auto rounded-2xl border border-white/10 bg-card p-6"
    >
      <VueTypography variant="TitleB" as="h2" class="mb-2.5"
        >Keybinds</VueTypography
      >
      ...
      <template v-else>
        ...
        >
          No keybinds found — this mod's INI has no "; Constants" section, or
          none of its [Key...] sections have a value set yet.
        </VueTypography>
        ...
        <template v-if="persistVars.length > 0">
          ...
          <div v-for="pv in persistVars" :key="pv.name" class="mb-4.5">
            <VueSelect
              ...
              :label="pv.name"
              :options="
                pv.options.map((opt) => ({ label: String(opt), value: opt }))
              "
            ...
            <template v-else>
              ...
              <VueInput
                ...
                :disabled="savingVars[pv.name]"
                @change="
                  saveVar(pv.name, ($event.target as HTMLInputElement).value)
                "
              />
            </template>
            <VueTypography
      ...
      <div class="flex items-center justify-end">
        <VueButton type="button" variant="outlined" @click="emit('close')"
          >Close</VueButton
        >
      </div>
    </div>
