...
<template>
  <div class="fixed inset-0 z-100 flex items-center justify-center bg-black/60" @click.self="emit('close')">
    <form class="w-11/12 max-w-120 max-h-[85vh] overflow-y-auto rounded-2xl border border-white/10 bg-card p-6" @submit.prevent="handleSubmit">
      <VueTypography variant="TitleB" as="h2" class="mb-2.5">Edit Mod</VueTypography>

      <div class="mb-5 flex items-center gap-4">
        <img :src="imageDataUrl ?? '/images/placeholder.jpg'" alt="" class="size-20 rounded-2xl border border-white/10 object-cover" />
        <VueButton type="button" variant="outlined" size="sm" @click="pickImage">Choose New Image</VueButton>
      </div>

      <VueInput id="mod-name" v-model="form.name" label="Name" container-class="mb-4.5" required />
      ...
      <div class="mb-4.5">
        <VueSelect v-model="selectedTarget" label="Category" :options="categoryOptions" placeholder="Uncategorized" />
        <div class="mt-2.5 flex items-center justify-start">
          <VueButton type="button" variant="outlined" size="sm" :disabled="!canMove || isMoving" @click="handleMove">
