<template>
  <div :class="cn('flex flex-col gap-2', containerClass)">
    ...
    <div class="relative flex">
      ...
      <input
        ...
        :class="
          cn(
            ...
            'rounded-lg border border-white/10 bg-white/5 text-foreground',
            ...
            'disabled:cursor-not-allowed disabled:border-white/5 disabled:bg-white/5 disabled:text-muted-foreground disabled:placeholder:text-white/20',
      ...
      <div
        v-if="$slots.otherIcon"
        class="inset-y-0 right-0 ml-2 flex items-center"
      >
        <slot name="otherIcon" />
      </div>
    </div>
...
const effectiveId = computed(
  () => (rawAttrs.id as string | undefined) ?? generatedId,
);
...
const ariaInvalid = computed(
  () =>
    props.error ||
    (props.required && touched.value && isEmptyValue(props.modelValue)),
);
