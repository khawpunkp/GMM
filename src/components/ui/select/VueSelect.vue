...
const isBelowMinSearchLength = computed(() => props.minSearchLength > 0 && debouncedSearch.value.length < props.minSearchLength);

const minSearchLengthText = computed(() => props.minSearchLengthText ?? `Type at least ${props.minSearchLength} characters to search`);
...
const filteredOptions = computed(() => {
  ...
  return props.options.filter((option) => normalize(option.label).includes(normalize(searchLower))).slice(0, 100);
});
...
<template>
  <SelectRoot data-slot="select" v-bind="forwarded" :open="isOpen" @update:open="onOpenUpdate" v-slot="{ open: slotOpen }">
    <div ref="selectTriggerRef" v-bind="attrs" :class="cn('w-full', containerClass, props.label ? 'flex flex-col gap-2' : '')">
      ...
      <SelectTrigger
        ...
        :clearable="clearable"
        :loading="loading"
        :has-value="props.modelValue !== undefined && props.modelValue !== null && props.modelValue !== ''"
    ...
    <SelectContent :align="align">
      ...
      <SelectGroup>
        ...
        <template v-else>
          <SelectItem v-for="option in filteredOptions" :key="option.value" :value="option.value" :disabled="option.disabled">
            {{ option.label }}
          </SelectItem>
        </template>
      </SelectGroup>
