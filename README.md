...
- **⌨️ Keybind Viewer:** Quickly view keybinds defined within a mod's INI files (specifically looks for `key = ...` lines within `[Key.*]` sections _after_ a `; Constants` marker).
...
8. **Presets:**
   - Presets are specific to the _currently active_ game.
