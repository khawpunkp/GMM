...
pub fn list_groups(conn: &Connection, base_mods_path: &Path) -> Result<Vec<ModGroupWithMembers>, String> {
    let groups: Vec<(i64, String)> = {
        let mut stmt = conn.prepare("SELECT id, name FROM mod_groups ORDER BY name").map_err(|e| e.to_string())?;
    ...
    groups
        ...
        .map(|(id, name)| {
            ...
            Ok(ModGroupWithMembers { id, name, is_enabled, members })
        })
        .collect()
}
...
pub fn get_group(conn: &Connection, base_mods_path: &Path, group_id: i64) -> Result<ModGroupWithMembers, String> {
    let name: String = conn
        .query_row("SELECT name FROM mod_groups WHERE id = ?1", params![group_id], |row| row.get(0))
        .map_err(|e| e.to_string())?;
    let members = fetch_members(conn, base_mods_path, group_id)?;
    let is_enabled = group_is_enabled(&members);
    Ok(ModGroupWithMembers { id: group_id, name, is_enabled, members })
}
...
pub fn create_group(
    ...
    tx.execute("INSERT INTO mod_groups (name) VALUES (?1)", params![name])
        .map_err(|e| e.to_string())?;
...
pub fn rename_group(conn: &Connection, group_id: i64, name: &str) -> Result<(), String> {
    let name = name.trim();
    if name.is_empty() {
        return Err("Group name cannot be empty.".to_string());
    }
    conn.execute("UPDATE mod_groups SET name = ?1 WHERE id = ?2", params![name, group_id])
        .map_err(|e| e.to_string())?;
    Ok(())
}
...
mod tests {
    ...
    fn create_and_toggle_group_with_mixed_state() {
        ...
        let group = create_group(&mut conn, &base, "My Outfit", &ids[0..2]).expect("create should succeed");
    ...
    fn create_group_rejects_already_grouped_mod() {
        ...
        create_group(&mut conn, &base, "First Group", &ids[0..2]).unwrap();
        let result = create_group(&mut conn, &base, "Second Group", &[ids[0], ids[2]]);
    ...
    fn remove_member_auto_deletes_group_when_down_to_one() {
        ...
        let group = create_group(&mut conn, &base, "Trio", &ids).unwrap();
